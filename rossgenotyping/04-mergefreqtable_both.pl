		...
		if (/^chr.*/){
			...
			if ($commonline[$header{'AN'}] > $libs) { #to make sure more than 2 libraries for each tissue are represented.
				...
				if (exists $header{'GENE.REFGENE'}) {
					if ($commonline[$header{'FUNC.REFGENE'}] ne "exonic") {
						if ($commonline[$header{'FUNC.ENSGENE'}] eq "exonic") { #change to ensembl annotation if not exonic
						...
						} else { #else maintain NCBI annotation
