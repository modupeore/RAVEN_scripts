	...
	while (<COMMON>){ #reading the input file
		if (/^chr.*/){
			...
			if ($commonline[$header{'AN'}] > $libs) { #to make sure more than 2 libraries for each tissue are represented.
				...
				if (exists $header{'GENE.ENSGENE'}) {
					$ANN{$commonline[$header{'CHROM'}]}{$commonline[$header{'POS'}]} = "$commonline[$header{'GENE.ENSGENE'}]";
					if ($commonline[$header{'FUNC.ENSGENE'}] eq "exonic"){
						$FUNC{$commonline[$header{'CHROM'}]}{$commonline[$header{'POS'}]} = "$commonline[$header{'EXONICFUNC.ENSGENE'}]";
					}else {
						$FUNC{$commonline[$header{'CHROM'}]}{$commonline[$header{'POS'}]} = "$commonline[$header{'FUNC.ENSGENE'}]";
					}
				}
...
print OUT "mergeALT\t";
if (exists $header{'GENE.ENSGENE'}) { print OUT "GENE\tFUNC\t"; }
print OUT "avgAF\tmedAF\tavgnew-AF\tmednew-AF\n";
print OUT2 "CHROM\tPOS\tREF\tmergeALT\t";
if (exists $header{'GENE.ENSGENE'}) { print OUT2 "GENE\tFUNC\t"; }
...
print OUT3 "CHROM\tPOS\tREF\tmergeALT\t"; if (exists $header{'GENE.ENSGENE'}) { print OUT3 "GENE\tFUNC\t"; } print OUT3 "avgnew-AF\n";
print OUT4 "CHROM\tPOS\tREF\tmergeALT\t"; if (exists $header{'GENE.ENSGENE'}) { print OUT4 "GENE\tFUNC\t"; } print OUT4 "avgnew-AF\n";
print OUT5 "CHROM\tPOS\tREF\tmergeALT\t"; if (exists $header{'GENE.ENSGENE'}) { print OUT5 "GENE\tFUNC\t"; } print OUT5 "avgnew-AF\n";
print OUT6 "CHROM\tPOS\tREF\tmergeALT\t"; if (exists $header{'GENE.ENSGENE'}) { print OUT6 "GENE\tFUNC\t"; } print OUT6 "avgnew-AF\n";
print OUT7 "CHROM\tPOS\tREF\tmergeALT\t"; if (exists $header{'GENE.ENSGENE'}) { print OUT7 "GENE\tFUNC\t"; } print OUT7 "avgnew-AF\n";
print OUT8 "CHROM\tPOS\tREF\tmergeALT\t"; if (exists $header{'GENE.ENSGENE'}) { print OUT8 "GENE\tFUNC\t"; } print OUT8 "avgnew-AF\n";
	...
	foreach my $position (sort {$a <=> $b} keys %{ $COMPARE{$chrom} } ) {
		if (exists $ALTCOMPARE{$chrom}{$position}) { #if filter(s) has at least PASS
			...
			if ($afaverage > 0.99) {	 #homo alt
				...
				if (exists $header{'GENE.ENSGENE'}){ print OUT3 "$ANN{$chrom}{$position}\t$FUNC{$chrom}{$position}\t"; }
				...
				if (exists $header{'GENE.ENSGENE'}){ print OUT4 "$ANN{$chrom}{$position}\t$FUNC{$chrom}{$position}\t"; }
				print OUT4 "$afaverage\n";
				$homoref++;
			} else {
				print OUT5 "$chrom\t$position\t$REFCOMPARE{$chrom}{$position}\t$ALTCOMPARE{$chrom}{$position}\t";
				if (exists $header{'GENE.ENSGENE'}){ print OUT5 "$ANN{$chrom}{$position}\t$FUNC{$chrom}{$position}\t"; }
			...
			if ($afmedian > 0.99) { #homo alt
				...
				if (exists $header{'GENE.ENSGENE'}){ print OUT6 "$ANN{$chrom}{$position}\t$FUNC{$chrom}{$position}\t"; }
				...
				if (exists $header{'GENE.ENSGENE'}){ print OUT7 "$ANN{$chrom}{$position}\t$FUNC{$chrom}{$position}\t"; }
				print OUT7 "$afaverage\n";
				$medhomoref++;
			} else {
				print OUT8 "$chrom\t$position\t$REFCOMPARE{$chrom}{$position}\t$ALTCOMPARE{$chrom}{$position}\t";
				if (exists $header{'GENE.ENSGENE'}){ print OUT8 "$ANN{$chrom}{$position}\t$FUNC{$chrom}{$position}\t"; }
